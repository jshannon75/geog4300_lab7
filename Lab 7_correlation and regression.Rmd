...
**Overview:**
This lab introduces you to NHGIS and the type of analysis you may be doing in your final project this semester. You'll also be applying techniques for correlation and OLS regression. Think of this as the first taste of your final project.
...
library(sf)
library(tidyverse)
census_data<-read_csv("data/nhgis0052_ds215_20155_2015_cd113th-114th.csv")
census_shape<-st_read("data/districts_2012.geojson", quiet=TRUE) %>% 
...
**Question 1 (2 points)** _Create a table for each county showing only the GISJOIN,STATE and rate variables listed above. Call the header of the resulting table when done._
...
**Question 2 (3 points)** _Join the district boundaries to the table from question 1.  Then scale the rates to create z scores. This will create a consistent range of values across variables for the next step of the process. Call the head of the table when done._
...
Next, you'll be assessing the correlation of these variables. We'll want to remove counties in states with very few disticts (2 or less), as gerrymandering options are limited in these states

**Question 5 (3 points):** _Filter out any states with 2 or less districts--you can use the STATEFP or STATE variables to group them. Then assess the normality of your five variables of interest (four census variables and the polsby_scale variable) using qq plots. Explain how your results suggest the use of parametric or non-parametric statistical tests._ 
...
**Question 6 (2 points)** _Create a data frame with just your five variables of interest (census rates plus polsby_scale), and remove the geometry column (hint: see the st_set_geometry() function from earlier this semester). Then use rcorr to create a correlation matrix with the appropriate measure (Spearman or Pearson). Remember that you'll need to convert the dataframe to a matrix._

```{r}
#Code goes here.
```

**Question 7 (2 points)** _Interpret the results your correlation matrix shows. Thinking about magnitude, direction, and significance, what variables are mostly closely correlated? What variables are most strongly associated with the polsby_scale variable?_
...
**Question 8 (3 points)** *Create an OLS regression model from the data you used for question 5, using the scaled Polsby compactness score as the dependent variable and the others as dependent variables. Call a summary of the model so your results appear in the response document.*
...
**Question 12 (3 points)** *Based on your results, assess how either the spatial/statistical distribution of residuals or tests for heterosketasticity may show a problem with this model.*

{Response goes here}

**Extra credit (2 points):**
Create another visualization of the polsby_scale variable that shows its distribution by state or its association with one or more census variables. Write a few sentences to explain what you think this visualization communicates.